...
public class WebhookProcessingRoute extends RouteBuilder {
    ...
    // Mapeamento de status EBANX → domínio
    // Referência: https://developers.ebanx.com/api-reference/webhooks
    // Statuses: CO=Confirmed/Paid, CA=Cancelled, PE=Pending, OP=Processing, RE=Refunded, CB=Chargeback
    private static final Map<String, PaymentStatus> EBANX_STATUS_MAP = Map.of(
            "CO", PaymentStatus.RECEIVED,      // Confirmed and paid
            "CA", PaymentStatus.CANCELLED,     // Cancelled
            "PE", PaymentStatus.PENDING,       // Pending
            "OP", PaymentStatus.PROCESSING,    // Processing (in analysis)
            "RE", PaymentStatus.REFUNDED,      // Refunded
            "CB", PaymentStatus.CHARGEBACK_REQUESTED  // Chargeback initiated
    );
    ...
    public void configure() {
        ...
        // ===== Rota EBANX Webhook =====
        // EBANX envia operation_type=payment_status_change com payment.status (CO, CA, RE, CB, etc.)
        from("direct:ebanx-webhook")
                .routeId("ebanx-webhook-processor")
                .log("Processando webhook EBANX: ${body}")
                .process(exchange -> {
                    JsonNode payload = exchange.getIn().getBody(JsonNode.class);
                    // EBANX envia o hash do pagamento e o status no campo payment
                    String pspPaymentId = payload.path("payment").path("hash").asText();
                    String statusCode   = payload.path("payment").path("status").asText();

                    PaymentStatus newStatus = EBANX_STATUS_MAP.get(statusCode);
                    if (newStatus == null) {
                        log.debug("Status EBANX ignorado: {}", statusCode);
                        exchange.setProperty("skip", true);
                        return;
                    }
